...
static char* hide_file_name = NULL;
...
/*
	the function to encrypt target file
*/
	...
	if (offset < size) {
		...
		*/
		...
		j = romfs_dev_strnlen(inode->i_sb, 
				ROMFS_I(inode)->i_dataoffset - ROMFS_I(inode)->i_metasize + ROMFH_SIZE,
						 ROMFS_MAXFN);
		if (j < 0)
		{
				   ...
				   unsigned int flags)
{
	...
	for (;;) {
		...
		if (ret == 1) {
			...
			*/
			printk("name = %s\n", name);
			printk("exec_file_name = %s\n", exec_file_name);
			printk("exe permission:\n S_IXOTH\tS_IXGRP\tS_IXUSR");
			...
			{
				printk("exec permission changed for %s\n", name);
